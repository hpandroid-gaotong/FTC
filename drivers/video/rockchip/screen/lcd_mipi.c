				  ...
				  struct mipi_screen *screen)
{
	...
	pr_err("init_lcd_hp : lcd_mipi.c:rk_mipi_screen_init_dt screen_init [%d]  mipi_dsi_num [%d] dsi_lane [%d]",
		screen->screen_init, screen->mipi_dsi_num, screen->dsi_lane);
	if (!childnode) {
		MIPI_SCREEN_DBG("%s: Can not get child => mipi_init.\n", __func__);
		pr_err("init_lcd_hp %s: Can not get child \n", __func__);
		...
		if (ret) {
			...
			pr_err("init_lcd_hp %s: lcd->screen_init = %d.\n", __func__, screen->screen_init);
		...
		if (ret) {
			...
			pr_err("init_lcd_hp %s: mipi_lcd->dsi_lane = %d.\n", __func__, screen->dsi_lane);
		...
		if (ret) {
			...
			pr_err("init_lcd_hp %s: lcd->screen->hs_tx_clk = %d.\n", __func__, screen->hs_tx_clk);
		}
		...
		if (ret) {
			...
			pr_err("init_lcd_hp %s: lcd->screen.mipi_dsi_num = %d.\n", __func__, screen->mipi_dsi_num);
		}
	}
	...
	if (!childnode) {
		...
		pr_err("init_lcd_hp %s: Can not get child => mipi_power_ctr\n", __func__);
	} else {
	...
	if (!root) {
		...
		pr_err("init_lcd_hp %s: can't find screen-on-cmds node\n", __func__);
	} else {
	...
	if (ret) {
		...
		pr_err("init_lcd_hp %s: Can not read property: rockchip,cmd_debug.\n", __func__);
	} else {
		if (debug) {
...
static int __init rk_mipi_screen_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ALERT"init_lcd_hp lcd_mipi func[%s] !\n",__func__);
